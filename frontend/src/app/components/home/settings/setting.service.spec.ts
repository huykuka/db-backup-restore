...
import settingService, { settingsInitialState, useSettings } from './settings.service';
...
describe('SettingService', () => {
    ...
    it('should fetch settings and update state', async () => {
        ...
        (apiClient.get as vi.Mock).mockResolvedValue({ data: { data: { settings: mockSettings } } });
    ...
    it('should update DB settings and show success toast', async () => {
        ...
        expect(apiClient.post).toHaveBeenCalledWith('settings/update/db-settings', mockSettings, undefined);
    });

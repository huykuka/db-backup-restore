        ...
        <div className="grid gap-4 p-4">
          {links.map((link: NavBarItem) => (
            <NavLink
              key={link.to}
              to={link.to}
              className={`text-sm font-medium text-blue-900 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-50 ${
                location.pathname === link.to
                  ? 'bg-gray-200 dark:bg-blue-950 p-1 px-3 rounded-md'
                  : 'hover:underline'
              }`}
            >
              <span className="align-middle font-medium flex text-gray-900 dark:text-gray-50 text-sm">
                {link.label}
              </span>
            </NavLink>
          ))}
